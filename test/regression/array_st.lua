...
do
  local function compare(x) return function(y)
...
do
  local function byKey(x) return function(y)
    if x.k < y.k then return -1 elseif x.k > y.k then return 1 else return 0 end
  end end
  local function fromOrdering(o) return o end
...
if failures > 0 then
  error(failures .. " regression check(s) failed")
end
