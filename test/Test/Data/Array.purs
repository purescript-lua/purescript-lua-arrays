...
testArray = do
  ...
  assert $ A.insertBy (flip compare) 1.5 [ 1.0, 2.0, 3.0 ] ==
    [ 1.0, 2.0, 3.0, 1.5 ]
  ...
  log
    "tail should return a Just-wrapped array containing all the items in an array apart from the first for a non-empty array"
  ...
  log
    "init should return a Just-wrapped array containing all the items in an array apart from the first for a non-empty array"
  ...
  log
    "uncons should split an array into a head and tail record when there is at least one item"
  ...
  log
    "unsnoc should split an array into an init and last record when there is at least one item"
  ...
  log
    "(!!) should return Just x when the index is within the bounds of the array"
  assert $ [ 1, 2, 3 ] !! 0 == (Just 1)
  assert $ [ 1, 2, 3 ] !! 1 == (Just 2)
  assert $ [ 1, 2, 3 ] !! 2 == (Just 3)

  log
    "(!!) should return Nothing when the index is outside of the bounds of the array"
  ...
  assert $ (A.findMap (\x -> if x > 3 then Just x else Nothing) [ 1, 2, 4 ]) ==
    Just 4
  assert $ (A.findMap (\x -> if x > 3 then Just x else Nothing) [ 1, 2, 1 ]) ==
    Nothing
  assert $ (A.findMap (\x -> if x > 3 then Just x else Nothing) [ 4, 1, 5 ]) ==
    Just 4
  ...
  log
    "intersperse should return the original array when given an array with zero or one elements"
  ...
  assert $ A.concatMap doubleAndOrig [ 1, 2, 3 ] == A.concat
    (map doubleAndOrig [ 1, 2, 3 ])
  ...
  assert $
    A.mapMaybe (\x -> if x /= 0 then Just x else Nothing) [ 0, 1, 0, 0, 2, 3 ]
      == [ 1, 2, 3 ]
  ...
  assert $ A.sort [ defined 1, undefined, defined 2 ] ==
  ...
  log
    "sortBy should not reorder elements that are equal according to a comparison function"
  ...
  log
    "sortWith should not reorder elements that are equal according to a projection"
  ...
  assert $ A.group [ 1, 2, 2, 3, 3, 3, 1 ] ==
    [ nea [ 1 ], nea [ 2, 2 ], nea [ 3, 3, 3 ], nea [ 1 ] ]

  log "groupAll should group equal elements into arrays"
  assert $ A.groupAll [ 1, 2, 2, 3, 3, 3, 1 ] ==
    [ nea [ 1, 1 ], nea [ 2, 2 ], nea [ 3, 3, 3 ] ]

  log
    "groupBy should group consecutive equal elements into arrays based on an equivalence relation"
  assert $ A.groupBy (\x y -> odd x && odd y) [ 1, 1, 2, 2, 3, 3 ] ==
    [ nea [ 1, 1 ], nea [ 2 ], nea [ 2 ], nea [ 3, 3 ] ]
  ...
  assert $ A.groupAllBy (comparing Down) [ 1, 3, 2, 4, 3, 3 ] ==
    [ nea [ 4 ], nea [ 3, 3, 3 ], nea [ 2 ], nea [ 1 ] ]
  ...
  log
    "nubBy should remove duplicate items from the list using a supplied predicate"
  ...
  assert $ A.unionBy (\_ y -> y < 5) [ 1, 2, 3 ] [ 2, 3, 4, 5, 6 ] ==
    [ 1, 2, 3, 5, 6 ]
  ...
  assert $ A.intersect [ 1, 2, 3, 4, 3, 2, 1 ] [ 1, 1, 2, 3 ] ==
    [ 1, 2, 3, 3, 2, 1 ]
