...
testArrayST = do
  ...
  log
    "poke should return false when attempting to modify a value outside the array bounds"
  ...
  log
    "poke should do nothing when attempting to modify a value outside the array bounds"
  ...
  log
    "toAssocArray should return all items in the array with the correct indices and values"

  assert $ all (\{ value: v, index: i } -> v == i + 1) $ ST.run
    ( do
        arr <- STA.thaw [ 1, 2, 3, 4, 5 ]
        STA.toAssocArray arr
    )

  assert $ all (\{ value: v, index: i } -> v == (i + 1) * 10) $ ST.run
    ( do
