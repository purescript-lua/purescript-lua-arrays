...
Notable changes to this Lua fork of `purescript-arrays` are recorded here. The fork
...
- `ST` `unshiftAll`/`splice` and `NonEmpty` `traverse1` FFI corrected.

## v7.4.0 - 2026-06-15

### Fixed

- Replaced `table.pack`/`table.unpack`/`table.move` (Lua 5.2+) with 5.1-safe
  equivalents, using an overlap-safe `move`.
